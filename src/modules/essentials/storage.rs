...
pub(crate) fn load_tx_summary_v2(
    ...
    Some(tx_summary_from_parts(
        txid_arr,
        packed.height,
        packed.traces,
        packed.outflows,
        packed.tx_type,
        packed.marketplace_info,
    ))
}

fn tx_summary_from_pointer_blob(blob: TxPointerBlobV3) -> AlkaneTxSummary {
    tx_summary_from_parts(
        blob.txid,
        blob.height,
        blob.traces,
        blob.outflows,
        blob.tx_type,
        blob.marketplace_info,
    )
}
...
fn tx_summary_from_parts(
    ...
    AlkaneTxSummary { txid, traces, outflows, height, tx_type, marketplace_info }
}
