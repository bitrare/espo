...
impl EssentialsProvider {
    ...
    /// Get full mempool alkane transactions with balance_changes (similar to get_alkane_block_txs_full)
    pub fn rpc_get_mempool_alkane_txs_full(
        &self,
        params: RpcGetMempoolAlkaneTxsFullParams,
    ) -> Result<RpcGetMempoolAlkaneTxsFullResult> {
        let page = params.page.unwrap_or(1).max(1) as usize;
            ...
            tx_renders
                ...
                .filter(|render| {
                    render.summary.as_ref().map_or(false, |s| {
                        // Check outflows (balance changes) - this captures simple transfers
                        let has_outflow = s.outflows.iter().any(|entry| {
                            entry.outflow.contains_key(filter_alk)
                        });
                        if has_outflow {
                            return true;
                        }
                        
                        // Also check traces for contract interactions
                        let filter_str = format!("{}:{}", filter_alk.block, filter_alk.tx);
...
pub struct RpcGetMempoolAlkaneTxsFullParams {
    pub page: Option<u64>,
    pub limit: Option<u64>,
